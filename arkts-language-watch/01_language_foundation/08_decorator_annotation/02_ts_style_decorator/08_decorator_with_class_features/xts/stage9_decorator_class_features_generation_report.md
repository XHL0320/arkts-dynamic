# Stage 9 — Decorator with Class Features — Generation Report
...
## Design Decisions
1. **Method/property decorator form**: Uses 2-param `(target, key)` form. Descriptor/equivalent structure is pending — 3-param support not confirmed.
2. **Override stability**: override syntax is pending — smoke files include fallback note. If unstable, downgrade to boundary.
3. **Implements stability**: interface/implements + decorator is pending — smoke includes fallback note.
4. **Generic class**: Not generated as .ets — only tracked in compiler candidate as pending.
5. **Abstract/accessor/overload/nested/expression/private**: All placed in boundary with no strong assertions.
6. **Field initializer**: Ordinary class feature contrast — not PR10632 regression.
7. **Fail_compile**: Each file contains exactly one primary expected error point.

## Coverage Matrix Updates
The following coverage IDs will be updated from "否" to "created":
...
## Pending Items
- Override syntax stability confirmation → may downgrade smoke to boundary
- Implements + decorator stability confirmation → may downgrade smoke to boundary
- Method decorator descriptor/equivalent structure confirmation → may adjust to 3-param
- Generic class + decorator support confirmation → may create smoke
- Readonly + decorator support confirmation → may downgrade pass to boundary
- Private member + decorator support confirmation → may downgrade boundary to fail_compile
