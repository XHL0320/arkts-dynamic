# Compiler Expected Candidate â€?Inheritance Override + Method Decorator
...
- `xts/fail_compile/decorator_class_feature_override_parameter_mismatch_fail_compile.ets`
- `xts/fail_compile/decorator_class_feature_override_return_mismatch_fail_compile.ets`
...
- Override syntax stability pending â€?may need boundary fallback

### Fail Compile
- `override value(x: string)` with base `value(x: number)` â†?parameter type mismatch error
- `override value(): string` with base `value(): number` â†?return type mismatch error
- Each fail_compile file has single expected error point

### Potential Compiler Issues
- If override syntax is not stable in current ArkTS â†?downgrade smoke to boundary
- If method decorator on override method is not supported â†?compile error
- If inherited decorated method resolution is incorrect â†?semantic bug
