...
|---|---|---|---|---|---|
| 01 | BigInt.asIntN(-1, 1n) | 待确认 | BigInt.asIntN(-1, 1n) | P1 | 初始规范点 |
| 02 | BigInt.asUintN(-1, 1n) | 待确认 | BigInt.asUintN(-1, 1n) | P1 | 初始规范点 |
| 03 | width 超出安全整数 | 待确认 | width 超出安全整数 | P1 | 初始规范点 |

## ArkTS Boundary

| 场景 | ECMA 行为 | ArkTS 静态语言预期 | ArkTS 动态/运行时预期 | 备注 |
|---|---|---|---|---|
| BigInt width RangeError | 运行时异常或解析失败 | 待确认是否前移为编译期错误 | 待确认 ArkRuntime 行为 | 初始记录 |
